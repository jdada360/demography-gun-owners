...
In 2017, there is not an explicit variable for this so we define it based on `q31`.
  ...
  mutate(
    sample = 
      ...
      factor(
        levels = 1:2,
        labels = 
          c("General population", "Gun Owner")
        )
    )%>% 
  set_variable_labels(
...
st(
  clean %>% filter(sample == "Gun Owners"),
  vars = demovars,
...
```{r, results = "asis", warning = F, message = F}
st(
  clean %>% filter(sample == "General population"),
  vars = demovars,
  group.weights = "weight1",
  summ = summstats,
  summ.names = summnames,
