...
For 2013, 2015 we restrict to gen pop sample and then merge with all other years.

We keep only weight1 for this analysis. If weight1 is missing, we drop.

```{r}
gen <-
  merge %>% 
  filter(
    !(year %in%  2013:2015 & is.na(weight1)),
    !is.na(weight1)
    ) %>% 
  select(-c(weight2, weight3)) %>% 
