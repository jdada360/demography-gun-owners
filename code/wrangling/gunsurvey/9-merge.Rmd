...
For 2017 and 2019, there are no gun owner/non-gun owner specific weight (i.e., normalised within groups), so we have to use the gen pop weights for the analysis of gun owners. If we want to compare gun owners to non-gunowners, we have to use the gen-pop weights. So we fill `weight` with `weight3` for those years.

There will be missing values for weight for years < 2019 because it does not include the gun owner augment (which creates a weight that is non-missing for all observations, i.e., `weight2`, and then scales that weight to be used for within gun-owner analysis, i.e., `weight3`)
...
Note that missing values for `weight` in 2013 and 2015 are normal.
