...
For 2017 and 2019, only weight1 is available, which we use.

For 2021-, weight1 is the correct full sample weight.
...
gen <-
  ...
  mutate(
    weight = 
      case_when(
        year == 2013:2015 ~ weight2,
        
      )
  ) %>% 
  filter(!is.na(weight)) %>%  
  select(-c(weight1, weight2, weight3, weight4, keep))
```
...
gen %>% 
  ...
  group_by(year) %>% 
  summarise(
    rate = sum(g*weight, na.rm = T)/sum(weight, na.rm = T)
  )
```

Online sources suggest gun ownership should be around 29\% in 2019 but it is around 36% in this data.

2017 matches online sources at around 30\%.

2013 is a lot lower than online sources at around 33\% so perhaps restricting to remove missing values from weight 1 is enough.

Allegedly This data is supposed to give a personal gun ownership of ~39–40\% in 2017
...
For 2013 and 2015, we must restrict the sample to gun-owners

```{r}
gun <-
  merge %>% 
  filter(
    !(year %in% 2013:2015 & sample != "Gun Owners")
    ) 
```
...
For 2017 and 2019, there are no gun owner/non-gun owner specific weight (i.e., normalized within groups), so we have to use the gen pop weights for the analysis of gun owners. If we want to compare gun owners to non-gun owners, we have to use the gen-pop weights. So we fill `weight1` with `weight3` for those years.

In 2015, there are 4 observations that are gun owners but have missing values for all weights. We drop them.

After the weight is fixed, we drop any missing values for `weight3.`
...
If a gun owner refuses to answer to the question about what kind of gun they own in the years the question is asked, we code this as saying that they own another type 
of gun.
